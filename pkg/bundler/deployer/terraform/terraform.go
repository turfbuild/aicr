...
type Generator struct {
	...
	// ClusterRollover emits a null_resource keyed on the cluster endpoint
	...
	//
	// Off by default, and deliberately so: current Terraform
	// deferred-action builds refuse a replace_triggered_by whose referent
	// is itself deferred, which is exactly the shape the unknown-cluster-
	...
	if g.ClusterRollover {
		notes = append(notes,
			"Cluster rollover is enabled: replacing the cluster replaces every release. "+
				"This is incompatible with deferred-action engines, which refuse a replace_triggered_by on a deferred resource.")
	}

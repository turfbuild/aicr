...
| `terraform` | Generates a Terraform/OpenTofu root module (`terraform plan` / `apply` / `destroy`, or `tofu`). One module call per release; `depends_on` carries the recipe's `dependencyRefs` exactly. Supports `--dynamic` via per-release `cluster-values.yaml`. Does not create a cluster: the `helm` provider is configured from variables. Requires the `terraform` or `tofu` binary at deploy time. |

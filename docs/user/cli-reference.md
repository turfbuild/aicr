...
| `--terraform-child-module` | | bool | Emit the bundle as a child module rather than a standalone root module: no provider configuration and no cluster-connection variables, so the calling configuration owns the provider and the module calls inherit it (`--deployer terraform` only, rejected elsewhere). Use this when the cluster is declared in the same configuration as the bundle — a module carrying its own provider block cannot take `depends_on`, `count` or `for_each`, and some engines refuse to walk one at all. |
| `--terraform-cluster-rollover` | | bool | Emit a containment shim in each component module so replacing the cluster replaces every release, instead of leaving state that describes objects in a cluster that no longer exists (`--deployer terraform` only, rejected elsewhere). Off by default, and incompatible with binding the bundle to a cluster created in the same configuration: deferred-action engines refuse a `replace_triggered_by` whose referent is itself deferred. |
...
| `terraform` | Generates a Terraform/OpenTofu root module (`terraform plan` / `apply` / `destroy`, or `tofu`). One module call per release; `depends_on` carries the recipe's `dependencyRefs` exactly. Supports `--dynamic` via per-release `cluster-values.yaml`. Does not create a cluster: the `helm` provider is configured from variables, or — with `--terraform-child-module` — by the calling configuration. Requires the `terraform` or `tofu` binary at deploy time. |
